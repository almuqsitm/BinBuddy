...
const Login = () => {
  ...
  const { login, signup } = useStore() // We'll update the store to handle this
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      // Navigation is handled by the checkSession/useStore state change implicitly,
      // but we can force it based on the role we just got/set
      // (The store updates `userRole` on success)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4 font-sans">
      <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl w-full max-w-md border border-gray-100 transition-all">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Role Selection (Visible for both or just Sign Up? Let's keep for both for easy dev switching) */}
